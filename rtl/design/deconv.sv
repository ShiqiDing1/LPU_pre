module deconv #(
    ...
    assign pipe_en_in = (state == ST_PROC) && (!need_read_input || in_valid);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            mac_array_data_vld <= (state == ST_PROC) && pipe_en_in;
            first_d           <= is_fst_kh_kw_fi;
            valid_pos_d       <= valid_pos;
        end
    end
    ...
    always_comb begin
        for (int i = 0; i < P_ICH; i++) begin
            x_vec[i] = valid_pos_d? line_buffer_rdata[i*A_BIT+:A_BIT]:'0 ;
        end
    end
    ...
    logic [P_ICH:0] last_delay;

always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        last_delay <= '0;
    end else if (out_ready) begin
        last_delay[0] <= pipe_en_in && is_lst_kh_kw_fi;
...
assign out_valid = last_delay[P_ICH];
endmodule
