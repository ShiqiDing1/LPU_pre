...
`ifdef PERF_TEST
    localparam int unsigned PERF_BATCH_SIZE = 2;
    localparam int unsigned N_OH = (N_IH + 2 * PAD - K) / STRIDE + 1;
    localparam int unsigned N_OW = (N_IW + 2 * PAD - K) / STRIDE + 1;

    perf_monitor #(
        .THEORETICAL_CYCLES(PERF_BATCH_SIZE * N_OH * N_OW * K * K *
                            (N_ICH / P_ICH) * (N_OCH / P_OCH))
    ) perf_mon (
        .clk        (clk),
        .rst_n      (rst_n),
