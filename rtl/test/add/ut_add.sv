    ...
    initial begin
        ...
        begin
            ...
            while ($fread(
                ...
                in1_queue.push_back(word_data_swapped[P_CH*A_BIT-1:0]);
            end
            $fclose(fd);
        ...
        begin
            ...
            while ($fread(
                ...
                word_data_swapped = {<<8{word_data}};
                golden_queue.push_back(word_data_swapped[A_BIT-1:0]);
            end
            $fclose(fd);
        ...
        begin
            ...
            while ($fread(
                ...
                word_data_swapped = {<<8{word_data}};
                in2_queue.push_back(word_data_swapped[P_CH*A_BIT-1:0]);
            end
            $fclose(fd);
...
`endif
        ...
        for (int i = 0; i < golden_queue.size(); i++) begin
            ...
            if (actual == expected) begin
                $display("Output[%0d] correct: expected=%0d (0x%0h), actual=%0d (0x%0h)", i, expected, expected,
                         actual, actual);
            end else if (actual === 'x) begin
                $display("Output[%0d] missing: expected=%0d (0x%0h), actual=undefined", i, expected, expected);
                error_count++;
            end else if (actual === 'z) begin
                $display("Output[%0d] high-impedance: expected=%0d (0x%0h), actual=high-Z", i, expected,
                         expected);
                error_count++;
            end 
            if (actual !== expected) begin
...
`ifdef PERF_TEST
    localparam int unsigned PERF_VECTOR_COUNT = 100;

    perf_monitor #(
        .THEORETICAL_CYCLES(PERF_VECTOR_COUNT)
    ) perf_mon (
        .clk        (clk),
        .rst_n      (rst_n),
        .input_fire (in1_valid && in1_ready && in2_valid && in2_ready),
