"""Part 3: parallel EU synchronization exercise (addresses are complete)."""
...
def op_matmul_sync(left, right, bias, *, l1_tile=None, l0_tile=None):
    M_L2 = left.shape[0]
    N_L2 = right.shape[0]
    K_L2 = left.shape[1]
    ...
    task_gen = TaskGen("op_matmul_sync")
    ...
    bias_gm_addr = 0
    bias_gm_size = ceil_align(bias_aligned.size().numel() * bias_aligned.element_size(), BUS_WIDTH)
    left_gm_addr = bias_gm_addr + bias_gm_size
    left_gm_size = ceil_align(left_aligned.size().numel() * left_aligned.element_size(), BUS_WIDTH)
        ...
        for l1_m_start_in_l2 in range(0, M_L2, M_L1):
            ...
            for l1_k1_start_in_l2 in range(0, K1_L2, K1_L1):
                ...
                if ub_addr + left_bytes > UB_SIZE:
                    task_gen.set_flag(src=1, dst=0, id=0)
                    task_gen.wait_flag(src=1, dst=0, id=0)
                ...
                count_gdma_old = count_gdma
                count_gdma = (count_gdma + 1) % MAX_SETWAIT_ID
                ...
                task_gen.set_flag(src=0, dst=1, id=count_gdma_old)
                task_gen.wait_flag(src=0, dst=1, id=count_gdma_old)
                for l0_n_start_in_l1 in range(0, n_size_l1, N_L0):
                    ...
                    if pmb_addr + pmb_bytes > PMB_SIZE:
                        task_gen.set_flag(src=2, dst=0, id=0)
                        task_gen.wait_flag(src=2, dst=0, id=0)
                        pmb_addr_base = 0
                    else:
                    ...
                    task_gen.set_flag(src=0, dst=2, id=1)
                    ...
                    for l0_m_start_in_l1 in range(0, m_size_l1, M_L0):
                        ...
                        for l0_k1_start_in_l1 in range(0, k1_size_l1, K1_L0):
                            ...
                            if l0_n_start_in_l1 == 0 and l0_m_start_in_l1 == 0 and l0_k1_start_in_l1 == 0:
                                count_ldma_old = count_ldma
                                count_ldma = (count_ldma + 1) % MAX_SETWAIT_ID
                            ...
                            lmb_wrap = lmb_addr + lmb_bytes > LMB_SIZE
                            lmb_addr_base = 0 if lmb_wrap else lmb_addr
                            ...
                            rmb_wrap = rmb_addr + rmb_bytes > RMB_SIZE
                            rmb_addr_base = 0 if rmb_wrap else rmb_addr

                            if lmb_wrap:
                                task_gen.set_flag(src=2, dst=1, id=0)
                                task_gen.wait_flag(src=2, dst=1, id=0)
                            if rmb_wrap:
                                task_gen.set_flag(src=2, dst=1, id=1)
                                task_gen.wait_flag(src=2, dst=1, id=1)
                            ...
                            task_gen.set_flag(src=1, dst=2, id=count_ldma_old)

                            bias_en = (l0_k1_start_in_l1==0 and l1_k1_start_in_l2==0)
                            psum_en = l0_k1_start_in_l1 > 0 or l1_k1_start_in_l2 > 0

                            task_gen.wait_flag(src=1, dst=2, id=count_ldma_old)

                            if l0_m_start_in_l1 == 0 and l0_k1_start_in_l1 == 0:
                                task_gen.wait_flag(src=0, dst=2, id=1)

                            if flag_aru2mxu == True:
                                task_gen.wait_flag(src=3, dst=2, id=0)
                                flag_aru2mxu = False
                            ...
                            if output_en:
                                task_gen.set_flag(src=2, dst=3, id=0)
                                task_gen.wait_flag(src=2, dst=3, id=0)
                                ...
                                is_last_mxu = (l1_n_start_in_l2 + n_size_l1 >= N_L2) and (l1_m_start_in_l2 + m_size_l1 >= M_L2)
                                if psb_addr == psb_addr_max and not is_last_mxu:
                                    task_gen.set_flag(src=3, dst=2, id=0)
                                    flag_aru2mxu = True
            task_gen.fence(3)
    ...
    task_gen.generate()
    return {
        "taskbin": str(Path("op_matmul_sync.bin").resolve()),
