        ...
        for b in range(batch):
            for h in range(tile_h):
                for w in range(tile_w):
                    for c0 in range(C0):
                        local_h = start_tensor_h + h
                        local_w = start_tensor_w + w
                        moved_c = C0 * c1 + c0
                        local_c = start_tensor_c + C0 * c1 + c0
                         
                        if moved_c < tile_c:
                            ifm_C1BHWC0[c1, b, h, w, c0] = ifm[b, local_h, local_w, local_c]

    return ifm_C1BHWC0.reshape((C1, batch, tile_h, tile_w, C0))

def matmul_m1k1m0k0_n1k1n0k0(
    m1n1m0n0, m1k1m0k0, n1k1n0k0,
    bias_n1n0, deq_n1n0,
    M1, N1, K1, bias_en, psum_en, deq_en
):
                        ...
                        for ow in range(slice_ow):
                            ...
                            if (0 <= ih < slice_ih) and (0 <= iw < slice_iw):
                                for k0_idx in range(K0):
                                    local_c1 = start_tile_c1 + ic1
                                    local_h = start_tile_h + ih
                                    local_w = start_tile_w + iw
                
                                    ifm_m1k1m0k0[m1_idx][k1_idx][m0_idx][k0_idx] = ifm[local_c1, b, local_h, local_w, k0_idx]
                            else:
                                for k0_idx in range(K0):
                        ...
                        for slice_oh_start_in_tile in range(0, oh_size_tile, H_L0):
                            for slice_ow_start_in_tile in range(0, ow_size_tile, W_L0):
                                for slice_ci1_start_in_tile in range(0, ci1_size_tile, CI1_L0):
                                    ...
                                    psum_en = not bias_en
                                    deq_en =(slice_ci1_start_in_tile + slice_ci1_size == ci1_size_tile) and tile_ic_start_in_tensor + ic_size_tile == in_channel
                                    ...
                                    h_blocks = ceil_div(oh_size_tile, H_L0)
                                    w_blocks = ceil_div(ow_size_tile, W_L0)

                                    oc_idx = slice_oc_start_in_tile // CO_L0
                                    h_idx = slice_oh_start_in_tile // H_L0
                                    w_idx = slice_ow_start_in_tile // W_L0

                                    psb_addr = (
                                        oc_idx * h_blocks * w_blocks
                                        + h_idx * w_blocks
                                        + w_idx
                                    )
