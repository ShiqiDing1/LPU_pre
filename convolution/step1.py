...
def test_matmul():
    ...
    for m1 in range(M1):
        for n1 in range(N1):
            temp = np.zeros((M0, N0))
            for k1 in range(K1):
                temp += np.matmul(left_m1k1m0k0[m1, k1],right_n1k1n0k0[n1, k1].transpose(),)
            result_m1n1m0n0[m1][n1] = temp
    result_mn = result_m1n1m0n0
    result_mn = result_m1n1m0n0.transpose(0, 2, 1, 3).reshape(M1 * M0, N1 * N0)[:M, :N]
